...
class MassCsvLine:
	...
	def massnote(self):
		return MassNote(self.masstemplates.masstemplate(self.csv_file_line.first_field()), self.csv_file_line.after_first_fields())
...
class MassTemplates:
	...
	def masstemplate(self, name):
		if name not in self.template_by_names:
			raise Error('There is no mass template with name "{0}"'.format(name))
...
class CsvNoteFileName:
	...
	def __str__(self):
		...
		return os.path.join(self.folderpath, "{0}_{1}.csv".format(self.massdeck.name().replace(':', '_'), self.massnotetype.name()))
...
mass_templates = MassTemplates({ "da" : MassTemplate("Deutsch::Aussprache", "Deutsch Aussprache") })
